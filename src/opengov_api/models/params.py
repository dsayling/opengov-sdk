...
from .enums import RecordStatus, DocumentStepStatus
...
class ListDocumentStepsParams(BaseModel):
    ...
    Example:
        >>> from opengov_api.models import ListDocumentStepsParams, DocumentStepStatus, DateRangeFilter
        >>> from datetime import date
        >>>
        >>> # Simple filter
        >>> params = ListDocumentStepsParams(
        ...     filter_status=DocumentStepStatus.COMPLETE,
        ...     filter_record_id="record-12345"
        ... )
        >>>
        >>> # Date range filter
        >>> params = ListDocumentStepsParams(
        ...     filter_completed_at=DateRangeFilter(gt=date(2025, 3, 1)),
        ...     page_size=50
        ... )
    """

    # Filters
    filter_record_id: str | None = None
    filter_label: str | None = None
    filter_status: DocumentStepStatus | None = None
    filter_completed_at: date | datetime | DateRangeFilter | None = None
    filter_activated_at: date | datetime | DateRangeFilter | None = None
    ...
    def to_query_params(self) -> dict[str, Any]:
        ...
        if self.filter_record_id:
            params["filter[recordID]"] = self.filter_record_id
        if self.filter_label:
            params["filter[label]"] = self.filter_label
        if self.filter_status:
            params["filter[status]"] = self.filter_status.value

        # Date filters (can be simple date or range)
        for field_name, param_value in [
            ("completedAt", self.filter_completed_at),
            ("activatedAt", self.filter_activated_at),
        ]:
            if param_value:
                if isinstance(param_value, DateRangeFilter):
                    params.update(param_value.to_query_params(field_name))
